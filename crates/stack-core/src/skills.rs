...
pub fn register_skill(
    ...
    sync_codex_home_link(paths, &record)?;
...
fn sync_preinstalled_skills(paths: &StackPaths) -> Result<(), SkillError> {
    ...
    for skill_id in PREINSTALLED_SKILL_IDS {
        ...
        sync_codex_home_link(paths, registry.skills.last().unwrap())?;
    }
...
fn sync_codex_home_link(paths: &StackPaths, record: &SkillRecord) -> Result<(), SkillError> {
    let source_dir = resolve_skill_source_path(paths, &record.source_path)?
        .parent()
        .ok_or_else(|| SkillError::NotFound(record.skill_id.clone()))?
        .to_path_buf();
    let target_root = paths.codex_home.join("skills");
