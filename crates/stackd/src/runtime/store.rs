...
impl RuntimeStore {
    ...
    pub fn load_snapshot(&self) -> anyhow::Result<Option<FactorySnapshot>> {
        let conn = self.connect()?;
        let text: Option<String> = conn
            .query_row(
                "SELECT snapshot_json FROM factory_snapshot WHERE id = 1",
                [],
                |row| row.get(0),
            )
            .optional()?;
    ...
    pub fn save_snapshot(&self, snapshot: &FactorySnapshot) -> anyhow::Result<()> {
        let conn = self.connect()?;
        conn.execute(
            "INSERT INTO factory_snapshot (id, snapshot_json, updated_at) VALUES (1, ?1, ?2) ON CONFLICT(id) DO UPDATE SET snapshot_json = excluded.snapshot_json, updated_at = excluded.updated_at",
            params![serde_json::to_string(snapshot)?, snapshot.updated_at],
        )?;
        Ok(())
    }
