...
async fn fetch_runs(client: &Client, base_url: &str) -> Result<Vec<GepaRunCursor>, reqwest::Error> {
    match fetch_runs_from_path(client, base_url, "/runs?limit=12").await {
        Ok(runs) => return Ok(runs),
        Err(primary_error) => {
            if let Ok(runs) = fetch_runs_from_path(client, base_url, "/workspace").await {
                return Ok(runs);
            }
            if let Ok(runs) = fetch_runs_from_path(client, base_url, "/status").await {
                return Ok(runs);
            }
            Err(primary_error)
        }
...
async fn fetch_runs_from_path(
    client: &Client,
    base_url: &str,
    path: &str,
) -> Result<Vec<GepaRunCursor>, reqwest::Error> {
    let payload = client
...
fn run_values(payload: &Value) -> Vec<Value> {
    if let Some(values) = payload.as_array() {
        return values.clone();
    }
    for key in ["items", "runs", "run_requests"] {
...
impl GepaRunCursor {
    fn from_value(value: &Value) -> Option<Self> {
        let run_id = read_string(value, "runId")
            .or_else(|| read_string(value, "run_id"))
            .or_else(|| read_string(value, "id"))
            .or_else(|| read_string(value, "request_id"))?;
        let usage = value.get("usage").unwrap_or(&Value::Null);
        let error = value.get("error").unwrap_or(&Value::Null);
        Some(Self {
            run_id,
            request_id: read_string(value, "request_id"),
            status: read_string(value, "status").or_else(|| read_string(value, "request_status")),
            ...
            cost_usd: read_f64(value, "costUsd")
                .or_else(|| read_f64(value, "cost_usd"))
                .or_else(|| read_f64(usage, "cost_usd")),
            total_tokens: read_i64(value, "totalTokens")
                .or_else(|| read_i64(value, "total_tokens"))
                .or_else(|| read_i64(usage, "total_tokens")),
            ...
            error: read_string(value, "error")
                .or_else(|| read_string(error, "message"))
                .or_else(|| read_string(error, "reason")),
        })
    }
    ...
    fn progress_changed(&self, prior: &Self) -> bool {
        self.candidate_count != prior.candidate_count
    ...
    fn is_failed(&self) -> bool {
        ...
        matches!(
            status.as_str(),
            "failed" | "error" | "cancelled" | "canceled" | "stopped" | "expired" | "rejected"
        )
    }
}
