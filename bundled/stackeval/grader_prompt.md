...
- The grader runs before the reviewer stage. Do not penalize the packet because
  `grade.json` or `review.json` is absent or because a pre-finalize
  `acceptance.md` row says `SE-B77-6-LEVERAGE` is missing those files. Judge
  Stack leverage from the available trace, runtime, harness, and packet
  evidence; the finalizer will re-check grade/review presence after review.
...
{
  "status": "partial",
  "task_outcome_score": 3,
  "stack_leverage_score": 3,
  "confidence": "high",
  "score_summary": "one sentence",
  "evidence": ["relative or absolute paths"],
  "caveats": ["short caveat"],
  "release_guards": [
    {
      "kind": "MCP/API check | docs gate | harness check",
...
Write `grade.md` with a concise rationale and the highest-leverage Stack
improvement revealed by the packet. `grade.json` must be strict parseable JSON:
escape quotes inside strings and do not include Markdown fences in the file.
