...
usage() {
  printf '%s\n' \
    ...
    "  --harness-mode MODE cli | service (default: cli or STACKEVAL_GEPA_HARNESS_MODE)" \
...
FROM_STAGE="prepare"
SKIP_STAGES=""
HARNESS_MODE="${STACKEVAL_GEPA_HARNESS_MODE:-cli}"
PREPARE_ONLY=0
NO_GRADE=0
NO_HARNESS=0
...
case "${HARNESS_MODE}" in
  cli | service) ;;
  *) die "--harness-mode must be cli or service, got ${HARNESS_MODE}" ;;
esac
export STACKEVAL_GEPA_HARNESS_MODE="${HARNESS_MODE}"
