...
def harness_event(args: argparse.Namespace) -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "command": (
            "POST <optimizer_service>/runs"
            if harness_mode == "service"
            else "uv run --project <gepa_evals_project> synth-optimizers gepa run --config <gepa_config>"
        ),
...
def stackeval_run_id(config: dict[str, Any], packet_dir: Path) -> str:
    harness_path = packet_dir / "harness.json"
    if harness_path.is_file():
        try:
            harness = json.loads(harness_path.read_text())
            run_id = str(harness.get("run_id") or "").strip()
            if run_id:
                return run_id
        except Exception:
            pass
