...
${user_msg}"
  ...
  if ! python3 -m json.tool "${packet_dir}/grade.json" >/dev/null 2>"${packet_dir}/grade.json.error"; then
    log "grader wrote invalid grade.json; check ${packet_dir}/grade.json.error"
    write_pipeline_state "${packet_dir}" "grade" "failed" "invalid grade.json"
    return 1
  fi
