...
run_harness() {
  ...
  set +e
  if [[ "${STACKEVAL_GEPA_HARNESS_MODE:-cli}" == "service" ]]; then
    python3 "${SCRIPT_DIR}/run_gepa_service_harness.py" \
      --config-json "${config_json}" \
      --packet-dir "${packet_dir}" \
      --gepa-config "${gepa_config}" \
      --service-url "${STACK_OPTIMIZER_SERVICE_URL:-http://127.0.0.1:8879}" \
