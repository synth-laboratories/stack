...
const server = Bun.spawn(["bun", "run", "scripts/bombadil_tui_b0_server.ts"], {
  cwd: process.cwd(),
  env: {
    ...process.env,
...
try {
  ...
  console.log(`stack_bombadil_b0_runner_ok ${proof.message}`)
} finally {
...
async function waitForHealth(serverPort: number): Promise<void> {
  ...
  throw new Error(`Bombadil B0 bridge did not become healthy on port ${serverPort}`)
}
...
async function waitForPassingProof(): Promise<{ message: string }> {
  ...
  while (Date.now() < deadline) {
    ...
    if (proof && !proof.ok) {
      throw new Error(`Bombadil B0 proof failed: ${proof.message ?? proof.failedScenario ?? "unknown"}`)
    }
    await sleep(500)
  }
...
async function readProof(): Promise<
  { ok: boolean; message?: string; failedScenario?: string } | undefined
> {
  try {
    const raw = await readFile(proofPath, "utf8")
    const parsed = JSON.parse(raw) as { ok?: unknown; message?: unknown; failedScenario?: unknown }
    if (typeof parsed.ok !== "boolean") return undefined
    return {
      ok: parsed.ok,
      message: typeof parsed.message === "string" ? parsed.message : undefined,
      failedScenario: typeof parsed.failedScenario === "string" ? parsed.failedScenario : undefined,
    }
  } catch {
    return undefined
  }
