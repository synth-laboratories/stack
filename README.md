...
(`synth-optimizers`, StackEval) and hosted (`synth-ai`, `usesynth.ai`) — from
...
Stack is **open source** (MIT) and in **public alpha**. Install from source today;
first-party installer tarballs and Homebrew are planned — see
[docs/DISTRIBUTION.md](docs/DISTRIBUTION.md) and [docs/RELEASE.md](docs/RELEASE.md).

| Today | Planned |
| --- | --- |
| `git clone` + `make install` | First-party installer at `stack.usesynth.ai` |
| **dev** channel on `main` (`make bump-dev`) | **stable** tagged releases |
| GitHub source + release tarball | Homebrew tap (after support proof) |

Stable release floor in `version.json` (`release: 0.1.0`) is the intended first public
version line — dev builds show `stack --version` with channel and dev build id.

- **[Quality guide](docs/QUALITY.md)** — lint, Bombadil, acceptance tiers, StackEval
- **[Release process](docs/RELEASE.md)** — dev/nightly vs stable, versioning, changelog
...
- **[Synth productivity guide](docs/SYNTH_PRODUCTIVITY.md)** — OSS + hosted workflows

## Install

Requires [Bun](https://bun.sh) and the [Codex CLI](https://developers.openai.com/codex).
Hosted Synth features (SMR, Factory, hosted optimizers) need an API key from
**[usesynth.ai/keys](https://usesynth.ai/keys)** — set `SYNTH_API_KEY` in your environment
or in the path configured in `stack.config.json` (`environments.*.authEnvFile`).
...
Never commit API keys or tokens. Stack never reads `~/.codex/auth.json` and does not
write under the user's `~/.codex` home.
