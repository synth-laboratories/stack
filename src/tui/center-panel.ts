...
export function renderActiveProjectsStyled(input: ActiveProjectsRenderInput): StyledText {
  ...
  chunks.push(...styleRuntimeSnapshotLine(input.runtimeSnapshot, input.columns))
  chunks.push(fg(theme.fgPrimary)("\n"))
...
function styleRuntimeSnapshotLine(snapshot: StackdFactorySnapshot | null | undefined, columns: number): TextChunk[] {
  ...
  const text = oneLine(`runtime ${state} local:${localStatus} synth:${remoteAuth}${remoteRuns}${remoteOptimizers}${localRuns}${eventSeq}`, columns)
