...
export function stackSkillsRoot(stackRoot: string): string {
  return join(stackRoot, ".stack", "skills")
}
...
export function ensureStackSkills(stackRoot: string, codexHome = defaultCodexHome()): string[] {
  ...
  for (const skill of discoverStackSkills(stackRoot, { includeCodexHome: false, includePlugins: false })) {
    if (skill.origin !== "stack") continue
    const source = resolve(dirname(skill.sourcePath))
  ...
  return installed.length > 0 ? installed.sort() : synced
}
